...
export function ZionReels({
  ...
  return (
    <div className="reels-overlay">
      ...
      {storyOpen ? (
        <div className="story-viewer" onClick={() => setStoryOpen(null)}>
      ...
      {pendingUpload ? (
        <div className="upload-compose">
          <section>
            <header>
              <button onClick={cancelUpload} aria-label="Cancel upload"><X /></button>
              <div><b>New {pendingUpload.kind === "reel" ? "Reel" : "Story"}</b><small>Add a caption before sharing</small></div>
            </header>
            ...
            <label>
              Caption
              <textarea value={caption} onChange={(event) => setCaption(event.target.value)} maxLength={500} placeholder="Write a caption…" />
              <small>{caption.length}/500</small>
            </label>
            <button className="publish-upload" disabled={uploading} onClick={() => void confirmUpload()}>
              <Upload /> {uploading ? `Uploading ${progress}%` : pendingUpload.kind === "reel" ? "Share Reel" : "Share Story"}
            </button>
          </section>
        </div>
      ) : null}
