...
function FriendChat({ friendship, friend, user, onBack }: { friendship: Friendship; friend?: ZionProfile; user: User; onBack: () => void }) {
  const [messages, setMessages] = useState<FriendMessage[]>([]); const [text, setText] = useState(""); const [uploading, setUploading] = useState(false); const [friendOnline, setFriendOnline] = useState(false); const [friendTyping, setFriendTyping] = useState(false); const listRef = useRef<HTMLDivElement>(null); const fileRef = useRef<HTMLInputElement>(null); const liveRef = useRef<RealtimeChannel | null>(null); const typingStopRef = useRef<number | null>(null); const remoteTypingRef = useRef<number | null>(null);
  ...
  useEffect(()=>{if(!supabase||!friend?.id)return;const client=supabase;const channel=client.channel(`friend-live-${friendship.id}`,{config:{presence:{key:user.id}}}).on("presence",{event:"sync"},()=>{const presence=channel.presenceState();setFriendOnline(Boolean(presence[friend.id]));}).on("broadcast",{event:"typing"},({payload})=>{if(payload.userId===user.id)return;setFriendTyping(Boolean(payload.typing));if(remoteTypingRef.current)window.clearTimeout(remoteTypingRef.current);if(payload.typing)remoteTypingRef.current=window.setTimeout(()=>setFriendTyping(false),2200);}).subscribe((status)=>{if(status==="SUBSCRIBED")void channel.track({userId:user.id,onlineAt:new Date().toISOString()});});liveRef.current=channel;return()=>{if(typingStopRef.current)window.clearTimeout(typingStopRef.current);if(remoteTypingRef.current)window.clearTimeout(remoteTypingRef.current);void client.removeChannel(channel);liveRef.current=null;};},[friend?.id,friendship.id,user.id]);
  const announceTyping = (typing:boolean) => { void liveRef.current?.send({type:"broadcast",event:"typing",payload:{userId:user.id,typing}}); };
  const changeText = (value:string) => { setText(value);announceTyping(Boolean(value.trim()));if(typingStopRef.current)window.clearTimeout(typingStopRef.current);typingStopRef.current=window.setTimeout(()=>announceTyping(false),1400); };
  const send = async () => { if(!supabase||!text.trim())return;const value=text.trim();setText("");announceTyping(false);await supabase.from("friend_messages").insert({friendship_id:friendship.id,sender_id:user.id,message:value});await load(); };
  ...
  return <div className="social-overlay"><section className="friend-chat"><header><button onClick={onBack}><ArrowLeft /></button><span className="chat-avatar">{friend?.avatar ?? "🙂"}<i className={friendOnline?"online":""}/></span><div><b>{friend?.username ?? "ZION friend"}</b><small>{friendTyping?"Typing…":friendOnline?"Online now · Permanent chat":"Permanent private chat"}</small></div></header><div className="moderation-banner compact"><ShieldAlert size={15} />Never send money, passwords, exact location or unwanted explicit media.</div><div className="friend-message-list" ref={listRef}>{messages.map((item)=><div key={item.id} className={item.sender_id===user.id?"friend-bubble mine":"friend-bubble theirs"}>{item.media_url&&item.media_type==="image"?<img src={item.media_url} alt="Shared attachment" />:null}{item.media_url&&item.media_type==="video"?<video src={item.media_url} controls playsInline />:null}{item.message?<span>{item.message}</span>:null}{item.sender_id===user.id?<small className={item.read_at?"read":""}>{item.read_at?"✓✓":"✓"}</small>:null}</div>)}{friendOnline?<div className={friendTyping?"presence-peek typing":"presence-peek"}><span>{friend?.avatar ?? "🙂"}</span>{friendTyping?<div className="typing-dots"><i/><i/><i/></div>:<small>Here</small>}</div>:null}</div><div className="friend-compose"><input ref={fileRef} hidden type="file" accept="image/*,video/mp4,video/webm,video/quicktime" onChange={(event)=>void upload(event.target.files?.[0])}/><button className="media-button" aria-label="Share photo or video" disabled={uploading} onClick={()=>fileRef.current?.click()}><ImagePlus /></button><input value={text} onChange={(event)=>changeText(event.target.value)} onKeyDown={(event)=>event.key==="Enter"&&void send()} placeholder={friendTyping?`${friend?.username??"Friend"} is typing…`:"Message your friend…"} maxLength={1000}/><button className="send-button" aria-label="Send message" onClick={()=>void send()}><Send /></button></div></section></div>;
}
