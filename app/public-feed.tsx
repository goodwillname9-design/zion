...
export default function PublicFeed({user,onClose,isAdmin=false}:{user:User;onClose:()=>void;isAdmin?:boolean}){
 ...
 const [discussion,setDiscussion]=useState<string|null>(null),[comments,setComments]=useState<{id:string;username:string;body:string;created_at:string}[]>([]),[comment,setComment]=useState(''),[commentBusy,setCommentBusy]=useState(false);
 const [reports,setReports]=useState<{id:string;username:string;body:string;reports:number}[]|null>(null);
 ...
 async function remove(p:Post){if(!supabase||!confirm('Delete this public post?'))return false;const {data,error}=await supabase.rpc('zion_feed_delete',{p_id:p.id});if(error){setError('Post could not be deleted.');return false;}setPosts(old=>old.filter(o=>o.id!==p.id));if(data){const result=await supabase.storage.from('zion-feed').remove([data]);if(result.error)setStatus('Post removed. Its uploaded file still needs storage cleanup.');}return true;}
 async function discuss(id:string,action:string,text=''){if(!supabase)return;setCommentBusy(true);try{const {data,error}=await supabase.rpc('zion_feed_discuss',{p_post:id,p_action:action,p_body:text});if(error)throw error;if(action==='report')setStatus('Report submitted for admin review.');else if(action==='reports')setReports(data);else{setComments(data);setDiscussion(id);if(action==='comment')setComment('');}}catch{setError('Could not save/load discussion. Check V68 feed setup; wait a few seconds before retrying.');}finally{setCommentBusy(false);}}
 ...
 {isAdmin&&<button onClick={()=>void discuss('00000000-0000-0000-0000-000000000000','reports')}>Review reports</button>}
 {reports&&<section className={styles.composer}><h2>Reported posts</h2><button onClick={()=>setReports(null)}>Close reports</button>{reports.length?reports.map(r=><p key={r.id}>@{r.username}: {r.body} · {r.reports} reports <button onClick={()=>void remove({id:r.id} as Post).then(deleted=>{if(deleted)setReports(old=>old?.filter(p=>p.id!==r.id)??null);})}>Delete post</button></p>):<p>No reported posts.</p>}</section>}
 ...
 <footer><button aria-pressed={p.liked} className={p.liked?styles.liked:''} onClick={()=>void like(p)}><Heart size={18} fill={p.liked?'currentColor':'none'}/>{p.likes} likes</button><button onClick={()=>void share(p)}><Share2 size={18}/> Share</button><button disabled={commentBusy} onClick={()=>{if(discussion===p.id)setDiscussion(null);else void discuss(p.id,'list');}}>Comments</button><button onClick={()=>{const reason=prompt('Why are you reporting this post?');if(reason?.trim())void discuss(p.id,'report',reason);}}>Report</button></footer>
 {discussion===p.id&&<section className={styles.composer}><small>Latest 50 comments</small>{comments.map(c=><p key={c.id}><b>@{c.username}</b> {c.body}</p>)}<form onSubmit={e=>{e.preventDefault();if(comment.trim()&&!commentBusy)void discuss(p.id,'comment',comment);}}><label htmlFor="feed-comment">Add a comment</label><textarea id="feed-comment" value={comment} maxLength={1000} onChange={e=>setComment(e.target.value)}/><button disabled={commentBusy||!comment.trim()}>Send comment</button></form></section>}</article>)}</div>
