-- Run after V73. Keeps account data, money, friendship and security rules.
begin;
...
begin
 if p_action='buy' then raise exception 'Weapons are disabled in social worlds';end if;
...
begin
 if p_action in ('fire','reload','shop','start') then raise exception 'Combat is disabled in social worlds';end if;
...
update public.zion_forest_rounds set ends_at=now() where ends_at>now();
commit;
